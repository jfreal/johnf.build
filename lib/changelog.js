...
async function readBounded(response) {
    ...
    if (Number.isFinite(declared) && declared > MAX_BYTES) {
        throw new Error(`response declares ${declared} bytes, over the ${MAX_BYTES} cap`);
    }
    ...
    for (;;) {
        ...
        if (total > MAX_BYTES) {
            await reader.cancel();
            throw new Error(`response exceeded the ${MAX_BYTES} byte cap`);
        }
        chunks.push(value);
    }
...
async function load(envName) {
    ...
    try {
        ...
        // portfolio that can't deploy. Only error.message is printed: Node puts
        // the request URL in fetch error causes, and that URL is a secret.
        console.warn(`[changelog] Could not read ${envName}: ${error.message}`);
        return [];
    }
